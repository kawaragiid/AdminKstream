...
function normalizeProfile(doc) {
  ...
  return {
    ...
    expiresAt: typeof doc.expiresAt === "string" ? doc.expiresAt : typeof doc.expiresAt?.toDate === "function" ? doc.expiresAt.toDate().toISOString() : null,
...
function mapUserRecord(userRecord, profile = {}) {
  ...
  return {
    ...
    displayName: profile.displayName ?? userRecord.displayName ?? userRecord.email ?? "User",
    photoURL: profile.photoURL ?? userRecord.photoURL ?? null,
    role: profile.role ?? claims.role ?? CUSTOMER_ROLE,
...
async function fetchUserDoc(firestore, uid) {
  ...
  const snapshot = await firestore.collection(FIRESTORE_COLLECTIONS.ADMIN_USERS).doc(uid).get();
...
export async function listUsers({ limit = 50, pageToken } = {}) {
  ...
  const profiles = firestore ? await Promise.all(result.users.map((user) => fetchUserDoc(firestore, user.uid))) : result.users.map(() => null);

  const users = result.users.map((user, index) => mapUserRecord(user, profiles[index] ?? {}));
...
export async function createAdminUser({ email, password, displayName, role = ADMIN_ROLES.ADMIN, plan = USER_PLANS.PREMIUM }) {
  if (!isFirebaseConfigured) {
    const mock = {
      uid: `mock-${Date.now()}`,
      email,
  ...
  const firestore = getFirestoreClient();
  if (firestore) {
    await firestore.collection(FIRESTORE_COLLECTIONS.ADMIN_USERS).doc(userRecord.uid).set({
      role,
      plan,
      displayName,
      email,
...
export async function updateUserSubscription(uid, { plan, isActive, expiresAt, extendDays }) {
  ...
  if (typeof extendDays === "number" && extendDays > 0) {
    ...
    if (isFirebaseConfigured) {
      ...
      const currExp = curr?.expiresAt ? (typeof curr.expiresAt?.toDate === "function" ? curr.expiresAt.toDate() : new Date(curr.expiresAt)) : null;
  ...
  const patch = {
    ...
    ...(newExpiresAt ? (isFirebaseConfigured ? { expiresAt: new Date(newExpiresAt) } : { expiresAt: newExpiresAt }) : {}),
