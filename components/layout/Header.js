  ...
  return (
    <header className="sticky top-0 z-50 flex min-h-[60px] sm:min-h-[72px] items-center justify-between gap-2 sm:gap-4 border-b border-slate-800/60 bg-slate-900/80 px-3 sm:px-4 py-2 sm:py-4 backdrop-blur-md md:px-8">
      <div className="space-y-0.5 sm:space-y-1">
        ...
        <h1 className="text-lg sm:text-xl font-semibold text-slate-100 md:text-2xl truncate">
      ...
      <div className="flex items-center gap-2 sm:gap-3">
          ...
          {navigation.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`rounded-lg px-2.5 sm:px-3 py-1.5 sm:py-2 text-xs font-medium whitespace-nowrap ${isActive ? "bg-primary-600 text-white" : "bg-slate-800/70 text-slate-300"}`}
              >
                {item.short}
              </Link>
            );
        ...
        <div className="flex items-center gap-2 sm:gap-3 rounded-full border border-slate-800/70 bg-slate-900/90 px-2 sm:px-3 py-1 text-sm">
          <span className="inline-flex h-7 w-7 sm:h-9 sm:w-9 items-center justify-center rounded-full bg-primary-600 text-xs sm:text-sm font-semibold">
            {session?.displayName ? session.displayName.slice(0, 2).toUpperCase() : "AK"}
          </span>
          <div className="hidden leading-tight sm:block">
            <p className="text-xs sm:text-sm font-medium text-slate-100 truncate max-w-[120px] sm:max-w-none">
              {session?.displayName ?? "Admin Kawaragi"}
            </p>
